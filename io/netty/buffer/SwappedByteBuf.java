...
implements ByteBuf {
    ...
    public ByteBuf ensureWritableBytes(int writableBytes) {
        this.buf.ensureWritableBytes(writableBytes);
        return this;
    }

    @Override
    public int ensureWritableBytes(int minWritableBytes, boolean force) {
        return this.buf.ensureWritableBytes(minWritableBytes, force);
    }
    ...
    public ByteBuffer[] nioBuffers(int offset, int length) {
        ByteBuffer[] nioBuffers = this.buf.nioBuffers(offset, length);
        for (int i = 0; i < nioBuffers.length; ++i) {
            nioBuffers[i] = nioBuffers[i].order(this.order);
        }
        return nioBuffers;
