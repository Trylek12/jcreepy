...
public final class Base64 {
    ...
    public static ByteBuf decode(ByteBuf src, int off, int len, Base64Dialect dialect) {
        ...
        int len34 = len * 3 / 4;
        ByteBuf dest = Unpooled.buffer(len34).order(src.order());
        int outBuffPosn = 0;
        byte[] b4 = new byte[4];
        int b4Posn = 0;
    ...
    private static int decode4to3(byte[] src, int srcOffset, ByteBuf dest, int destOffset, Base64Dialect dialect) {
        ...
        if (src[srcOffset + 2] == 61) {
            int outBuff2 = (DECODABET[src[srcOffset]] & 255) << 18 | (DECODABET[src[srcOffset + 1]] & 255) << 12;
            dest.setByte(destOffset, (byte)(outBuff2 >>> 16));
            return 1;
        }
        if (src[srcOffset + 3] == 61) {
            int outBuff3 = (DECODABET[src[srcOffset]] & 255) << 18 | (DECODABET[src[srcOffset + 1]] & 255) << 12 | (DECODABET[src[srcOffset + 2]] & 255) << 6;
